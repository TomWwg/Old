...
$(function() {
	...
	$('#table').grid({
		  colName: ['排班名称', '起始时分','结束时分','创建时间','修改时间'],
		  colModel: [
		    {name: 'name', index:'title',sortable:true, style: {'textAlign':'center'},
		    ...
		    {name: 'sHm', index:'content',sortable:false, style: {'textAlign': 'center'}},
		    {name: 'eHm', index:'content',sortable:false, style: {'textAlign': 'center'}},
	...
	$("#add_btn").click(function(){
		$.dialog({
            ...
            id: 'addframe',
            title: '添加排班模板',
            iframeSrc: top.pt.ctx+'/web/module/schedule/showDialog.action',
            buttons: [{
                text: '确定',
                'class': 'bPrimary',
...
function modifyDialog(dataId){
	$.dialog({
		...
		id : 'editframe',
		title : '修改排班模板',
		iframeSrc : top.pt.ctx+'/web/module/schedule/showDialog.action?entity.id='+dataId,
		buttons : [{
			text : '确定',
			'class' : 'bPrimary',
