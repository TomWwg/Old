...
$(function() {
	...
	$('#table').grid({
		  colName: ['角色名称', /*'到期时间',*/ '备注','更新时间','权限分配'],
		  colModel: [
		    ...
		    {name: 'name', index:'name',sortable:true, style: {'textAlign':'center'},
			  formart : function(dataRow, data, fn){
				 return '<a href="javascript:void(0); " onclick="assignMenuDialog('+data.id+')" style="text-decoration:underline;">'+'权限配置'+"</a>";
			  }
			}
		  ],
		  headerFixed : true,
	...
	$("#add_btn").click(function(){
		...
		$.dialog({
            ...
            id: 'addframe',
            title: '添加角色信息',
            iframeSrc: top.pt.ctx+'/web/module/role/showDialog.action',
            buttons: [{
                text: '确定',
                'class': 'bPrimary',
                "id": 'add_role_btn_ok',
...
//菜单权限分配
function assignMenuDialog(dataId){
	$.dialog({
        width: 800,
        height: 600,
        maybeRefresh: true,
        id: 'assignframe',
        title: '权限分配',
        iframeSrc: top.pt.ctx+'/web/module/role/assignMenuDialog.action?ids='+dataId,
        buttons: [{
            text: '确定',
            'class': 'bPrimary',
            "id": 'add_assign_btn_ok',
            click: function(dialog, callback){
            	$.ajax({
					url : 'assignMenu.action',
					data : top.frames["assignframe"].$("#showForm").serialize(),
					success : function(data, callback){
						if (data.success == true) {
							dialog.close();
							jAlert("权限分配成功", '提示', 'attention');
							$('#table').grid('reload');
						}else {
							jAlert("权限分配失败："+data.msg, '错误', 'error');
						}
					},
					dataType : "json"
				});
...
function modifyDialog(dataId){
	$.dialog({
		...
		id : 'editframe',
		title : '修改角色信息',
		iframeSrc : top.pt.ctx+'/web/module/role/showDialog.action?entity.id='+dataId,
		buttons : [{
			text : '确定',
			'class' : 'bPrimary',
			"id" : 'modify_role_btn_ok',
			click : function(dialog, callback) {
				$.ajax({
					url : 'saveForm.action',
