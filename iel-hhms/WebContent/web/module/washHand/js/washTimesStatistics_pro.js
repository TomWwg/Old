...
//导出数据
function excelData() {
	var url = top.pt.ctx+'/web/module/exportFun/timesOfWashHandPro.action';
...
function calcResult(data){
	...
	*/
	...
	var totalLiquid = data.total != ''?data.total:'0';
	var totalBottle = data.number != ''?data.number:'0';
	
	$("#statis_result_per_out").html(totalLiquid);
	$("#statis_result_total_bottle").html(totalBottle);
	
	if(times != '0' && (totalLiquid == '0' || totalBottle == '0')){
		$("#error_info").show();
	}else{
		$("#error_info").hide();
	}
	
//	var onePush = $(".perPush").val();
//	var oneBottle = $(".perBottle").val();
//	var pushValue = onePush * times;
//	var bottleValue = times*onePush/oneBottle;
//	if(pushValue && pushValue!=NaN){
//		$("#statis_result_per_out").html(pushValue);
//		$("#error_info").hide();
//	}else{
...
//		}
//	}
//	if(bottleValue && bottleValue!=NaN && bottleValue!=Infinity){
//		$("#statis_result_total_bottle").html(bottleValue.toFixed(2));
//		$("#error_info").hide();
//	}else{
...
function checkStaffWashTimesTable(startTime,endTime,depart){
	...
	if(table == null){
		var urlTable = "findFrequencyByStaffPro.action";
		table = $('#table').grid({
		  colName: ['员工姓名', '合计用量/mL', '不清洁时/mL','有限清洁时/mL','清洁时/mL'],
		  colModel: [
